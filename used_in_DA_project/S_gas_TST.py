...
def replace_freq_cutoff(nma_obj,cutoff):
    for i,fr in enumerate(nma_obj.freqs):
        if fr == 0:
            ...
            print('Warning: freq replaced w/ cutoff. If this is gas-phase w/o TST, consider re-running at tighter convergence criterion!')
...
if 'gas' in action:
    ...
    pf_gas = PartFun(nma_gas, [ExtTrans(cp=True), ExtRot(im_threshold=1.0)])
    ...
    print('%s %s %s %s %s' % (U0, ZPE, H/1000,S_gas, G/1000))
