...
if 'ads' in action:
    ...
    if 'TST' in action:
        Modes[0] = 'TST' #Visually double check that this is indeed the bond formation/breakage mode!
        Modes[1] = 'trans'
        Modes[2] = 'trans'
    else:
        Modes[0] = 'trans'
    ...
    print('%s %s %s %s %s' % (U0,ZPE,H,S,H-300.00*(S/1000.00)))
...
if 'gas' in action:
    ...
    #mu = pf_vib.chemical_potential(T)*Hartree_2_Joule[0]*Na[0]#Chemical Potential [J/mol]
    print('H@%s [kJ/mol] \t S@%s [J/mol/K] \t G@%s [kJ/mol]' %(T,T,T))
