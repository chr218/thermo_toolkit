...
if 'ads' in action:
    ...
    '''
    #Cutoff tuning single vib into Svib.
    for mode_n in range(18):
        #Construct nma object
        nma = NMA(mol,PHVA(constrained_atom_list))        
        for i,freq in enumerate(nma.freqs):
            if i == mode_n:
                continue
            else:
                nma.freqs[i] = 0
        print(nma.freqs)
        pf_vib = PartFun(nma, [])#Construct Harmonic Oscillator partition function. 
        Svib = pf_vib.entropy(T)*Hartree_2_Joule[0]*Na[0] # Vibrational Entropy [J/mol/K]
        print(Svib)
    '''
    ...
    for N,f in enumerate(nma.freqs):
        nma.freqs[0:N] = [0]*N
    ...
    #nma_HO_cutoff = replace_freq_cutoff(nma,81)#Apply cutoff. 'trans' freqs are skipped.
    ...
    S = S_2D_ZSA(Modes,nma,T,unconst_mass_sum/1000/Na[0],cutoff)#Obtain entropy.
